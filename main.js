...
function showGuide(step=0){
  ...
  $('#guide-demo').innerHTML=step===0?`<img class="demo-rock" src="${art.sprites.hold.toDataURL()}" alt=""><i class="demo-ring"></i><img class="demo-climber" src="${art.sprites.reach.toDataURL()}" alt=""><span class="demo-pad-button">A</span><span class="demo-stick">↔ ${settings.language==='en'?'BALANCE':'重心'}</span><span class="demo-label"></span>`:step===1?`<div class="demo-memory"><strong>3 + 2 = ?</strong>${imgHTML({art:'mushroom'})}</div><div class="demo-options"><span>4</span><span class="demo-correct">5</span><span>6</span></div><span class="demo-label"></span>`:`<img class="demo-flag" src="${art.sprites.flag.toDataURL()}" alt=""><strong class="demo-timer">65</strong><span class="demo-label"></span>`;
...
function showQuestion(){
  ...
  $('#answers').innerHTML=q.choices.map((ch,i)=>`<button class="answer ${math?'number-answer':'picture-answer'}" data-answer="${i}" data-pad="A" aria-label="${math?(settings.language==='en'?'Answer ':'答案 '):''}${math?ch.label:labelFor(ch,settings.language)}">${math?ch.value:imgHTML(ch)}</button>`).join('');
...
function renderHud(){if(!run)return;
  ...
  if(run.activeMemory!==seenActive){seenActive=run.activeMemory;if(seenActive>=0){const card=run.cards[seenActive];$('#memory-stage').innerHTML=`<div class="memory-card ${card.category==='math'?'calculation-card':''}"><span class="card-number">0${seenActive+1}</span>${card.category==='math'?`<div class="forest-equation">${card.equation}<span>= ?</span></div>`:''}<div class="forest-object">${imgHTML(card)}</div><i class="exposure-track"></i></div>`;sound.chime('memory');}else{$('#memory-stage').innerHTML='<div class="memory-empty"><span class="seed">✦</span><p>'+t(run.shown.length===6?'memoryPassed':'memoryNext')+'</p></div>';}}
